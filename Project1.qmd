...
plot.state <- function(df,var_name="state_data.Enrollment_Value"){
   ...
   print(plot_data)
  ggplot(data=plot_data, aes(x=state_data.Year,y= y_axis,color=state_data.division)) + geom_smooth() 
}

plot.state(Data_Combined$noncounty_data,)
...
plot.county <- function(df,var_name="county_data.Enrollment_Value",state = " AL ",
                        direction ="Top", n=5){
   plot_data <- df|> filter(county_data.state == state)|>
    summarize(y_axis=mean(get(var_name))) |>
     arrange(ifelse(direction=="Top",y_axis,desc(y_axis)))
   
   print(plot_data)
}

plot.county(Data_Combined$county_data,)
